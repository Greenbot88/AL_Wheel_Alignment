...
void autoMode()
{
   ...
   if((startFlag == 0 ) && (buttonStateJackFwdRevControl == INPUT_OFF) && (buttonStateGantryHomePos == INPUT_ON) && \ 
    ...
    if ((buttonStatePhotoSensor == HIGH) && (buttonStateJackFwdRevEnd == INPUT_ON)) //&& (buttonStateJackUpDnEnd == INPUT_ON) && (buttonStateCrossOverEnd == INPUT_ON)
...
{  
  ...
  while((buttonStateGantryControl  == INPUT_OFF) && (buttonStateCrossOverControl == INPUT_OFF) && (buttonStateJackUpDnControl == INPUT_OFF) && (buttonStateJackFwdRevControl == INPUT_OFF) \
   ...
   {
    testingInPut();
    Serial.println("Jackfwdcontrol ON");
    if(buttonStateForwardButton == INPUT_ON)
     {
      onJackForward();
     Serial.println("Jackfwdcontrol fwd on");
     }
     if(buttonStateReverseButton == INPUT_ON)
     {
      onJackReverse();
      Serial.println("Jackfwdcontrol reverse on");
     }
     if((buttonStateForwardButton == INPUT_OFF) && (buttonStateReverseButton == INPUT_OFF) )
     {
       testingInPut();
       offJack();
       Serial.println("Jackfwdcontrol OFF");
     }    
   }
...
//        onGantryReverse();  
  ...
  if ((buttonStateGantryControl  == INPUT_OFF) && (buttonStateCrossOverControl == INPUT_OFF) && (buttonStateJackUpDnControl == INPUT_OFF) && (buttonStateJackFwdRevControl == INPUT_OFF) \
   ...
   {
    testingInPut();
    Serial.println("JF ON");
    if(buttonStateForwardButton == INPUT_ON)
     {
      onJackForward();
      Serial.println("JF FWD ON");
     }
     if(buttonStateReverseButton == INPUT_ON)
     {
      onJackReverse();
      Serial.println("JF BCK ON");
     }
     if((buttonStateForwardButton == INPUT_OFF) && (buttonStateReverseButton == INPUT_OFF) )
     {
       testingInPut();
       offJack();
       Serial.println("JF OFF");
     }    
   }
   /* Jack Up down Control*/
...
void loop() {
  // put your main code here, to run repeatedly:
    //tower_test();
//    if(buttonStateJackFwdRevControl == INPUT_ON)
//    {
//      Serial.println("Jack Fwd CTRL");
//      }
    /*while (1)
    {
    if (RS232.available())
       ...
       case 'S':       
        ...
        case 'Z':     
              ...
              while(array_stringTcp[1] != '2')
              {
                ...
                if((array_stringTcp[1] == '2') || ( millis() - currentMillis >= 3000)) //termination cmd '2' or 3sec whichever comes first will offjackfwd
                {
                  Serial.println("Jack OFF");
                  offJack();
...
void tower_test()
{
  digitalWrite( pinQ2_5_TOWER_LAMP_RED, HIGH);
  delay(1000);
digitalWrite(pinQ2_6_TOWER_LAMP_GREEN , HIGH);
delay(1000);
digitalWrite(pinQ0_3_TOWER_LAMP_YELLOW ,HIGH);
delay(1000);
//digitalWrite( pinQ2_3_TOWER_LAMP_BUZZER , HIGH);
//  delay(1000);

digitalWrite( pinQ2_5_TOWER_LAMP_RED, LOW);
delay(1000);
digitalWrite(pinQ2_6_TOWER_LAMP_GREEN , LOW);
delay(1000);
digitalWrite(pinQ0_3_TOWER_LAMP_YELLOW ,LOW);
delay(1000);
//digitalWrite( pinQ2_3_TOWER_LAMP_BUZZER , LOW);
//  delay(1000);
  }
